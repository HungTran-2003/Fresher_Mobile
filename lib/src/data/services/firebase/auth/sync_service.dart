      ...
      for (final account in accounts) {
        try {
          ...
          for (final singleTaxId in account.taxIdOrIds) {
            final localAccount = hiveService.getAccount(
              singleTaxId,
              account.username,
            );
            ...
            if (localAccount == null ||
                !account.updatedAt.isAtSameMomentAs(localAccount.updatedAt)) {
              await hiveService.saveAccount(account);
            }
          }
        } catch (e) {
          debugPrint(
            "SyncService: Failed to merge account ${account.username}: $e",
          );
        }
      }

      debugPrint(
        "SyncService: Completed account synchronization successfully. Cache size: ${hiveService.getAllAccounts().length}",
      );
    } catch (e) {
      debugPrint(
        "SyncService: Account sync failed: $e. Using offline Hive cache.",
      );
    }
  }
}
